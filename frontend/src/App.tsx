...
export default function App(): ReactNode {
  ...
  // 插件列表变化时注册 qiankun 子应用（qiankun 内部去重，可重复调用）。
  useEffect(() => {
    if (typeof window === 'undefined') return;
  ...
  } else if (routePath === '/publish') {
    content = <PublishForm plugins={plugins} onPublish={handlePublish} onRefreshPlugins={refreshPlugins} />;
  } else {
    content = (
      <PluginList
        plugins={plugins}
